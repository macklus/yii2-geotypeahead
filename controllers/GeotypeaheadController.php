...
class GeotypeaheadController extends Controller {
    ...
    public function actionSearch() {
        ...
        if ($q) {
            $query = Yii::$app->db->createCommand('SELECT gn_geoname.geonameid AS locality_id, gn_geoname.name AS locality, gn_admin2Codes.geonameid AS province_id, gn_admin2Codes.name AS province, gn_countryinfo.geonameid AS country_id, gn_countryinfo.name AS country FROM gn_geoname LEFT JOIN gn_admin2Codes ON CONCAT(gn_geoname.country, ".", gn_geoname.admin1, ".", gn_geoname.admin2) = gn_admin2Codes.code LEFT JOIN gn_countryinfo ON gn_geoname.country = gn_countryinfo.iso_alpha2 WHERE gn_geoname.name LIKE :query AND gn_geoname.fcode LIKE "PPL%" ORDER BY gn_geoname.population DESC;');
            $my_search = "%{$q}%";
